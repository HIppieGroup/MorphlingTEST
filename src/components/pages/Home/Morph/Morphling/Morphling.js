...
export default class Morphling {
  ...
  staticAnimation = (balls, time, isNegative, intensive) => {
    for (let i = 0; i < balls.length; i += 1) {
      let realIntensive = this.time / 20 <= intensive ? this.time / 20 : intensive;
      if (this.isElipse) {
        realIntensive = 0.05;
      }
      const n = realIntensive * this.simplex.noise2D(
  ...
  handleIntersection = (isIntoOld, points) => {
    if (this.processing && !this.isElipse) {
      ...
      for (let i = nearPointIndex; i < lastPointIndexR; i += 1) {
        ...
        vx += (tx - this.balls[realI].x) * this.fadingPoints[pointIndex] * this.amp;
        vy += (ty - this.balls[realI].y) * this.fadingPoints[pointIndex] * this.amp;
        ...
        setTimeout(() => {// eslint-disable-line
          points[realI].setPos(// eslint-disable-line
